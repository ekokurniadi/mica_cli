  ...
  Future<void> generate(JsonParseModel parser) async {
    ...
    final map = {
      ...
      'datasources': List.from(parser.datasources.map((e) {
        final result = {
          'datasource_name':
              '${parser.featureName.snakeCase}$e'.replaceAll(' ', ''),
          'datasource_class_name':
              '${parser.featureName.titleCase}${e.toString().titleCase}'
                  .replaceAll(' ', ''),
          'datasource_field_name':
              '${parser.featureName.camelCase}${e.toString().titleCase}'
                  .replaceAll(' ', ''),
          'datasource_file_name':
              '${parser.featureName.snakeCase}_${e.toString().snakeCase}'
                  .replaceAll(' ', ''),
          'source': e,
        };
        return result;
